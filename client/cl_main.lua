...
function StartCreator()
    ...
    for i, m in pairs(overlay_all_layers) do
        ...
        { name = m.name, visibility = 0, tx_id = 1, tx_normal = 0, tx_material = 0, tx_color_type = 0, tx_opacity = 1.0,
            tx_unk = 0, palette = 0, palette_color_primary = 0, palette_color_secondary = 0, palette_color_tertiary = 0,
            var = 0, opacity = 0.0 }
    end
    MenuData.CloseAll()
...
function MainMenu(Target)
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'main_character_creator_menu',
        { title = QR.Texts.Appearance, subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
...
function OpenBodyMenu()
    ...
    local SkinToneOptions = { QR.Texts.Color1, QR.Texts.Color2, QR.Texts.Color3, QR.Texts.Color4, QR.Texts.Color5,
        QR.Texts.Color6 }
    local elements = {
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'body_character_creator_menu',
        ...
        if CreatorCache[data.current.category] ~= data.current.value then
            ...
            BodyFunctions[data.current.category](ped, CreatorCache)
        end
    end)
end
...
function OpenFaceMenu()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'face_main_character_creator_menu',
        { title = QR.Texts.Face, subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
...
function OpenHairMenu()
    ...
    if IsPedMale(ped) then
        ...
        table.insert(elements,
            { label = QR.Texts.HairStyle, value = CreatorCache["hair"].model or 0, category = "hair", desc = "",
                type = "slider", min = 0, max = #category, change_type = "model", id = a, options = options })
        a = a + 1
        options = {}
        ...
        table.insert(elements,
            { label = QR.Texts.HairColor, value = CreatorCache["hair"].texture or 1, category = "hair", desc = "",
                type = "slider", min = 1, max = GetMaxTexturesForModel("hair", CreatorCache["hair"].model or 1),
                change_type = "texture", id = a, options = options })
        options = {}
        a = a + 1
        ...
        table.insert(elements,
            { label = QR.Texts.BeardStyle, value = CreatorCache["beard"].model or 0, category = "beard", desc = "",
                type = "slider", min = 0, max = #category, change_type = "model", id = a, options = options })
        a = a + 1
        options = {}
        ...
        table.insert(elements,
            { label = QR.Texts.BeardColor, value = CreatorCache["beard"].texture or 1, category = "beard", desc = "",
                type = "slider", min = 1, max = GetMaxTexturesForModel("beard", CreatorCache["beard"].model or 1),
                change_type = "texture", id = a, options = options })
        options = {}
        a = a + 1
    else
        ...
        table.insert(elements,
            { label = QR.Texts.Hair, value = CreatorCache["hair"].model or 0, category = "hair", desc = "",
                type = "slider", min = 0, max = #category, change_type = "model", id = a, options = options }
        )
        a = a + 1
        options = {}
        ...
        table.insert(elements,
            { label = QR.Texts.Hair, value = CreatorCache["hair"].texture or 1, category = "hair", desc = "",
                type = "slider", min = 1, max = GetMaxTexturesForModel("hair", CreatorCache["hair"].model or 1),
                change_type = "texture", id = a, options = options }
        )
        options = {}
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'hair_main_character_creator_menu',
        { title = QR.Texts.Hair_beard, subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
        end, function(data, menu)
        MoveCharacterCreatorCamera( -560.133, -3780.92, 238.6)
        MainMenu()
        ...
        if data.current.change_type == "model" then
            if CreatorCache[data.current.category].model ~= data.current.value then
                ...
                HairFunctions[data.current.category](ped, CreatorCache)
            end
...
function OpenEyesMenu()
    MenuData.CloseAll()
    local ped = PlayerPedId()
    local EyesColorOptions = { QR.Texts.EyesTone1, QR.Texts.EyesTone2, QR.Texts.EyesTone3, QR.Texts.EyesTone4,
        QR.Texts.EyesTone5, QR.Texts.EyesTone5 }
    local elements = {
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'eyes_character_creator_menu',
        { title = QR.Texts.Eyes, subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
        end, function(data, menu)
        OpenFaceMenu()
    end, function(data, menu)
        if CreatorCache[data.current.category] ~= data.current.value then
            CreatorCache[data.current.category] = data.current.value
            EyesFunctions[data.current.category](ped, CreatorCache)
        end
    end)
end
...
function OpenEyelidsMenu()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'eyelid_character_creator_menu',
        { title = QR.Texts.Eyelids, subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
        end, function(data, menu)
        OpenFaceMenu()
    end, function(data, menu)
        if CreatorCache[data.current.category] ~= data.current.value then
            CreatorCache[data.current.category] = data.current.value
            EyelidsFunctions[data.current.category](ped, CreatorCache)
        end
    end)
end
...
function OpenEyebrowsMenu()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'eyebrows_character_creator_menu',
        { title = QR.Texts.Eyebrows, subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
        end, function(data, menu)
        OpenFaceMenu()
    end, function(data, menu)
        if CreatorCache[data.current.category] ~= data.current.value then
            CreatorCache[data.current.category] = data.current.value
            EyebrowsFunctions[data.current.category](ped, CreatorCache)
        end
    end)
end
...
function OpenNoseMenu()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'nose_character_creator_menu',
        { title = QR.Texts.Nose, subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
        end, function(data, menu)
        OpenFaceMenu()
    end, function(data, menu)
...
function OpenMouthMenu()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'mouth_character_creator_menu',
        { title = QR.Texts.Mouth, subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
        end, function(data, menu)
        OpenFaceMenu()
    end, function(data, menu)
...
function OpenCheekbonesMenu()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'cheekbones_character_creator_menu',
        { title = 'Cheek Bones', subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
        end, function(data, menu)
        OpenFaceMenu()
    end, function(data, menu)
...
function OpenJawMenu()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'jaw_character_creator_menu',
        { title = QR.Texts.Jaw, subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
        end, function(data, menu)
        OpenFaceMenu()
    end, function(data, menu)
...
function OpenEarsMenu()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'ears_character_creator_menu',
        { title = QR.Texts.Ears, subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
        end, function(data, menu)
        OpenFaceMenu()
    end, function(data, menu)
...
function OpenChinMenu()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'chin_character_creator_menu',
        { title = QR.Texts.Chin, subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
        end, function(data, menu)
        OpenFaceMenu()
    end, function(data, menu)
...
function OpenDefectsMenu()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'defects_character_creator_menu',
        { title = QR.Texts.Disadvantages, subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
        end, function(data, menu)
        OpenFaceMenu()
    end, function(data, menu)
...
function OpenMakeupMenu()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'makeup_character_creator_menu',
        { title = QR.Texts.Make_up, subtext = QR.Texts.Options, align = QR.Texts.align, elements = elements },
        function(data, menu)
        end, function(data, menu)
        MoveCharacterCreatorCamera( -560.133, -3780.92, 238.6)
        MainMenu()
...
RegisterNetEvent('qr_appearance:ApplySkin', function(SkinData, Target, ClothesData)
    CreateThread(function()
        local ped  = PlayerPedId()
        local _Target = Target or PedHasUseScenarioTask
